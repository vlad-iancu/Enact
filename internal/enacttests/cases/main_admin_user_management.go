...
func (c *mainAdminUserManagementCase) Run(t *utils.T) {
	...
	// Provider registration is administration too: it decides what every
	// user can connect to and stores the platform's client secret.
	if st := anon.DoJSON(t, http.MethodPost, "/admin/identity-providers/pat", strings.NewReader(`{}`), nil); st != http.StatusUnauthorized {
		t.Errorf("anonymous /admin/identity-providers/pat: got HTTP %d, want 401", st)
	}
	if st := anon.DoJSON(t, http.MethodPost, "/admin/identity-providers/oauth", strings.NewReader(`{}`), nil); st != http.StatusUnauthorized {
		t.Errorf("anonymous /admin/identity-providers/oauth: got HTTP %d, want 401", st)
	}
	...
	if st := user.DoJSON(t, http.MethodPost, "/admin/identity-providers/pat", strings.NewReader(`{"name":"nope"}`), nil); st != http.StatusForbidden {
		t.Errorf("non-admin provider registration: got HTTP %d, want 403", st)
	}

	// Provider administration: a PAT provider needs nothing but a name...
	...
	if st := admin.DoJSON(t, http.MethodPost, "/admin/identity-providers/pat", strings.NewReader(patBody), &patProvider); st != http.StatusCreated {
		t.Fatalf("admin register pat provider: got HTTP %d, want 201", st)
	}
	...
	// response tells the admin which redirect URI to register upstream.
	...
	status, raw := admin.DoJSONRaw(t, http.MethodPost, "/admin/identity-providers/oauth", strings.NewReader(oauthBody), &oauthProvider)
	if status != http.StatusCreated {
		t.Fatalf("admin register oauth provider: got HTTP %d, want 201", status)
	}
	...
	if st := admin.DoJSON(t, http.MethodPost, "/admin/identity-providers/pat", strings.NewReader(bad), nil); st != http.StatusBadRequest {
	...
	if st := admin.DoJSON(t, http.MethodPost, "/admin/identity-providers/pat", strings.NewReader(patBody), &dup); st != http.StatusConflict {
	...
	// Deleting them is administration as well.
	for _, name := range []string{adminPATProvider, adminOAuthProvider} {
		if st := admin.DoJSON(t, http.MethodDelete, "/admin/identity-providers/"+name, nil, nil); st != http.StatusNoContent {
			t.Errorf("admin delete provider %s: got HTTP %d, want 204", name, st)
		}
	}
	if st := admin.DoJSON(t, http.MethodDelete, "/admin/identity-providers/"+adminPATProvider, nil, nil); st != http.StatusNotFound {
	...
	if st := admin.DoJSON(t, http.MethodPost, "/admin/identity-providers/pat", strings.NewReader(identityProvider), nil); st != http.StatusCreated {
...
func (c *mainAdminUserManagementCase) TearDown(t *utils.T) {
	...
	for _, name := range []string{adminPATProvider, adminPATProvider + "-bad", adminOAuthProvider, adminIdentityProvider} {
		c.admin.DoJSON(t, http.MethodDelete, "/admin/identity-providers/"+name+"?force=true", nil, nil)
	}
}
