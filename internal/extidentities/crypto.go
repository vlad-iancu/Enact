...
// CryptoConfig carries the encryption key material. There is no default:
...
func NewVault(cfg CryptoConfig) (*Vault, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("extidentities: IDENTITIES_ENCRYPTION_KEY: %w", err)
	}
	...
	for i, old := range cfg.KeysOld {
		...
		if err != nil {
			return nil, fmt.Errorf("extidentities: IDENTITIES_ENCRYPTION_KEYS_OLD[%d]: %w", i, err)
		}
